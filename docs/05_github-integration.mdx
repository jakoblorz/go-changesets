...
│              SOURCE OF TRUTH                     │
├──────────────────────────────────────────────────┤
│  version.txt (PR branch) = Staged version        │
│  git tags (main branch)  = Published version     │
└──────────────────────────────────────────────────┘
```
...
- ✅ **Review gate** - Versions reviewed before publishing
- ✅ **Rollback safety** - Can abandon version PR without consequences
- ✅ **Clarity** - Clear separation between staging and production
...
- ✅ One or more Go modules with `go.mod` files
- ✅ `.changeset/` directory (auto-created by `changeset add`)
...
│   ├── auth/
│   │   ├── go.mod
│   │   ├── version.txt      # Created by version command
...
└── .github/
    └── workflows/
        ├── changeset.yml    # Main release workflow
        └── snapshot.yml     # RC workflow (optional)
```
...
# Prevent duplicate runs or out-of-order executions
concurrency:
...
jobs:
  ...
  version-prs:
    ...
    steps:
      ...
      - name: Capture current state # Capture tree BEFORE versioning (versioning deletes changesets)
        id: tree
        run: |
          changeset tree --format json > /tmp/tree.json
      ...
      - name: Create per-project PRs
        ...
        run: |
          changeset each --from-tree-file /tmp/tree.json --filter open-changesets -- sh -c '
            set -e
      ...
      - name: Link related PRs # Now that all PRs are created, we can re-render their bodies with RelatedPRs filled in
        if: success()
        env:
          GH_TOKEN: ${{ secrets.GITHUB_TOKEN }}
        run: |
          changeset each --from-tree-file /tmp/tree.json --filter open-changesets -- sh -c '
            ...
            echo "🔗 Linking related PRs for $PROJECT..."
            ...
            git clean -fd
          '
          echo "✅ Related PRs linked"

      - name: Close obsolete release PRs # In complex groups we may still be running the CI sequentially and would open PRs for projects that were already released by an earlier PR merge. Close those re-opened PRs.
        if: always()
        env:
          GH_TOKEN: ${{ secrets.GITHUB_TOKEN }}
        run: |
          ...
          echo "✅ Obsolete PR cleanup complete"
  ...
  publish:
    ...
    steps:
      ...
      - name: Summary
        if: steps.check_publish.outputs.has_outdated == 'true'
        run: |
          echo "## 🎉 Releases Published" >> $GITHUB_STEP_SUMMARY
          echo "" >> $GITHUB_STEP_SUMMARY
...
The workflow uses three built-in GitHub commands that handle all PR operations:
...
This saves relationship data for Step 2 (PR linking). Learn more about [changeset groups](./changeset-groups.mdx).

### Step 2: Create Version PRs
...
changeset each --filter open-changesets -- sh -c '
  ...
  # 3. Commit and push
  git add .
  git commit -m "chore: version $PROJECT to $(cat $PROJECT_PATH/version.txt)"
  git push -u origin "$BRANCH" --force
  
  # 4. Create PR with built-in command (handles template, mapping, everything)
  changeset gh pr open \
    --owner $OWNER \
    --repo $REPO
  
  # 5. Reset for next project (isolation!)
  git checkout main
  git reset --hard origin/main
  git clean -fd
'
...
**What `changeset gh pr open` does:**
- Reads current version from `version.txt`
- Renders PR body using template (`.changeset/pr-description.tmpl` or default)
- Creates new PR or updates existing one
- Stores PR mapping for later linking

### Step 3: Link Related PRs
...
This uses the tree data to:
- Find related projects in each changeset group
- Re-render each PR body with the related PRs list
- Update all PRs in the group

### Step 4: Close Obsolete PRs

For projects with no remaining changesets, close their release PRs:
...
This:
- Checks if a release PR exists for the project
- Closes the PR with a comment explaining why
- Deletes the release branch

### Step 5: Publish Releases
...
## GitHub PR Commands Reference

The `changeset gh pr` subcommands handle all release PR operations:

### `changeset gh pr open`

Creates or updates a release PR for a project.
...
- `--project`: Project name (auto-detected from env when run via `changeset each`)
- `--title`: PR title template (default: `🚀 Release {{.Project}} v{{.Version}}`)
- `--base`: Base branch (default: `main`)
- `--labels`: Comma-separated labels (default: `release,automated`)
- `--mapping-file`: Path to PR mapping file (default: `/tmp/pr-mapping.json`)

**Behavior:**
1. Reads current version from `version.txt`
2. Renders PR body using template (`.changeset/pr-description.tmpl` or default)
3. Creates new PR or updates existing one
4. Stores PR info in mapping file for linking

**Example:**
```bash
# In a changeset each context (auto-detects project)
...
Links related release PRs together using changeset tree data.
...
changeset gh pr link \
  ...
  --repo $REPO \
  --tree-file /tmp/tree.json \
  [--mapping-file /tmp/pr-mapping.json]
```

**Flags:**
- `--owner` (required): GitHub repository owner
- `--repo` (required): GitHub repository name
- `--tree-file` (required): Path to tree JSON from `changeset tree --format json`
- `--mapping-file`: Path to PR mapping file (default: `/tmp/pr-mapping.json`)

**Behavior:**
1. Reads tree data to find related projects
2. Reads mapping file to get PR numbers
3. Re-renders each PR body with related PRs list
4. Updates all PRs in related groups
...
Closes obsolete release PRs and deletes release branches.
...
- `--repo` (required): GitHub repository name
- `--project`: Project name (auto-detected from env when run via `changeset each`)
- `--comment`: Custom close comment

**Behavior:**
1. Checks if a release PR exists for the project
2. Closes the PR with an explanatory comment
3. Deletes the release branch

**Example:**
```bash
# In a changeset each context
changeset gh pr close --owner myorg --repo myrepo

# With custom comment
changeset gh pr close --owner myorg --repo myrepo --comment "No longer needed"
```

## Custom PR Templates

All three commands use templates for PR content. Create `.changeset/pr-description.tmpl` to customize:

```gotemplate
This PR was automatically generated for **{{.Project}}**.
...
{{range .RelatedPRs}}- #{{.Number}} Release {{.Project}} v{{.Version}}{{end}}
{{end}}
...
- `{{.Version}}`: New version
- `{{.CurrentVersion}}`: Current version before bump
- `{{.ChangelogPreview}}`: Formatted changelog preview
- `{{.RelatedPRs}}`: List of related PRs (for `gh pr link`)
- `{{.CommitSHA}}`: Commit that triggered the release
...
✅ **Independent review** - Each project reviewed and merged separately  
✅ **Selective releases** - Merge only projects ready for production  
✅ **Parallel workflows** - Multiple team members work on different releases  
✅ **Isolated failures** - One project's issues don't block others  
✅ **Clear history** - Each project has dedicated release commit  
✅ **Faster iterations** - Don't wait for all projects to be ready
...
CI creates 3 separate PRs:
  ├─ PR #123: Release auth v1.2.0
  ├─ PR #124: Release api v2.1.0
  └─ PR #125: Release shared v0.5.1

You can:
  - Merge auth PR → auth published
  - Hold api PR → needs more testing
  - Merge shared PR → shared published
```
...
**Tradeoffs:**
- ✅ Simpler (less shell scripting)
- ✅ All projects released together
- ❌ All-or-nothing (can't merge selectively)
- ❌ One project's issues block all others
...
```markdown
**Review checklist:**
- ✅ Version bump correct? (1.1.0 → 1.2.0 ✓)
- ✅ Changelog entry readable? (Yes ✓)
- ✅ Changesets consumed? (Yes ✓)
...
**After merge:**
- GitHub bot comments: "✅ Version PR created: #123"
- PR appears: "🚀 Release auth v1.2.0"
...
**Action:** Merge PR → Release automatically published!
...
- name: Check for changeset
  run: |
    ...
    if ! git diff origin/main --name-only | grep -q "^\.changeset/.*\.md$"; then
      echo "❌ No changeset found. Add one with: changeset add"
      exit 1
    fi
```
...
- name: Notify on Failure
  ...
  with:
    payload: |
      {
        "text": "⚠️ Release workflow failed! Check ${{ github.server_url }}/${{ github.repository }}/actions/runs/${{ github.run_id }}"
      }
  env:
    SLACK_WEBHOOK_URL: ${{ secrets.SLACK_WEBHOOK_URL }}
```
...
- name: Check changesets for secrets
  run: |
    if grep -r "ghp_\|github_pat_" .changeset/; then
      echo "❌ Potential secret found in changeset!"
      exit 1
    fi
```
...
- name: Comment on PRs
  run: |
    ...
    gh pr comment $PR_NUMBER --body "🎉 Released in auth@v1.2.0"
```
