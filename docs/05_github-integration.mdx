...
## Workflow steps (what and why)

- Checkout with full history (`fetch-depth: 0`) so tags and file history are available.
- Setup Go to install and run `changeset` reliably.
- `changeset-version`: captures `changeset tree` before versioning, versions each project in a worktree to avoid cross-project churn, opens/updates PRs, links related PRs, and closes obsolete PRs.
- `changeset-publish`: detects outdated versions (version file > tag) and publishes tags/releases.
- `changeset-snapshot`: validates the branch, checks for changesets, then creates RC tags/releases.
...
- Optional inputs: `version_token` (version PRs), `publish_token` (publish), `snapshot_token` (snapshots).
