...
The CLI includes built-in GitHub commands that handle PR creation, linking, and cleanup:

- `changeset gh pr open` - Create or update release PRs
- `changeset gh pr link` - Link related release PRs together
- `changeset gh pr close` - Close obsolete release PRs

These commands replace complex shell scripts with a simple, declarative workflow.
      ...
      - name: Create per-project PRs
        ...
        run: |
          changeset each --filter open-changesets -- sh -c '
            ...
            # Create/update PR with built-in command
