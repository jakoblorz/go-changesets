...
`go-changeset` looks for a **workspace root** by walking up from the current working directory until it finds a `go.work`.
...
- Projects store their version in `version.txt` at the project root.
